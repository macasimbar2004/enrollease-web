      ...
      child: Padding(
        ...
        child: ConstrainedBox(
          ...
          child: Builder(builder: (context) {
            final balanceAcc = widget.balanceAcc;
            return Form(
              key: formKey,
              child: Scrollbar(
                        ...
                        children: [
                          ElevatedButton(
                            onPressed: () async {
                              ...
                              if (result == null) {
                                final remainingBalance = FeesModel.fromMap(balanceAcc['remainingBalance']);
                                balanceAcc['remainingBalance'] = FeesModel(
                                  entrance: remainingBalance.entrance - (payment.amount?.entrance ?? 0),
                                  tuition: remainingBalance.tuition - (payment.amount?.tuition ?? 0),
                                  misc: remainingBalance.misc - (payment.amount?.misc ?? 0),
                                  books: remainingBalance.books - (payment.amount?.books ?? 0),
                                  watchman: remainingBalance.watchman - (payment.amount?.watchman ?? 0),
                                  aircon: remainingBalance.aircon - (payment.amount?.aircon ?? 0),
                                  others: remainingBalance.others - (payment.amount?.others ?? 0),
                                ).toMap();
